...
void Assembler::single_pass()
{
	...
	for (size_t i = 0; i < tokens.size(); i++)
	{
		...
		else if (Parser::resolve_type(current) == "INSTRUCTION") // instruction 
		{
			if (current_section->name == ".und" || current_section->name == ".abs")
			{
				throw Error("Error: instruction cannot be located inside .und or .abs section.");
			}
		...
		{
			if ((current_section->name == ".und" || current_section->name == ".abs") && (Parser::resolve_directive(current) == ".SKIP" || Parser::resolve_directive(current) == ".BYTE" || Parser::resolve_directive(current) == ".WORD"))
			{
				string error = "Error: directive cannot be located inside .und or .abs section -> " + Parser::resolve_directive(current) + " directive.";
